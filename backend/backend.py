...
Optionally, set HF_TOKEN_DEFAULT to provide a fallback when HF_TOKEN is unset.
"""
...
# ---------------------- CONFIG ----------------------
HF_TOKEN_DEFAULT = os.environ.get("HF_TOKEN_DEFAULT", "")
...
def _install_cloudflared():
    try:
        print("→ Installing cloudflared (if missing)…")
        subprocess.run(
            [
                "wget",
                "-q",
        ...
        subprocess.run(["apt-get", "-y", "-f", "install"], check=False)
    except Exception as exc:  # pragma: no cover - installer best effort
        print("⚠️ cloudflared download/install failed:", exc)


def _resolve_cloudflared_path():
    candidates = [
        os.environ.get("CLOUDFLARED", "").strip(),
        shutil.which("cloudflared") or "",
        "/usr/local/bin/cloudflared",
        "/usr/bin/cloudflared",
        "/bin/cloudflared",
    ]
    _fix_path_for_cloudflared()
...
def ensure_cloudflared():
    ...
    if not exe and os.path.exists("/usr/local/bin/cloudflared"):
        try:
            subprocess.run(["ln", "-sf", "/usr/local/bin/cloudflared", "/usr/bin/cloudflared"], check=False)
            exe = _resolve_cloudflared_path()
        except Exception:
            pass
    ...
    if not exe:
        raise RuntimeError(
            "cloudflared not found after install. Set CLOUDFLARED=/full/path/cloudflared or check permissions."
        )
    try:
        out = subprocess.check_output([exe, "--version"], text=True)
        print("✅ cloudflared ready:", out.splitlines()[0])
    except Exception:
        print("⚠️ cloudflared installed but version check failed (continuing)")
    return exe
...
def start_cloudflared(port=PORT, max_wait=60):
    ...
    cmd = [exe, "tunnel", "--url", f"http://127.0.0.1:{port}", "--no-autoupdate", "--loglevel", "info"]
...
def _json_errors(err):
    message = str(err)
    return jsonify({"error": message or "internal error"}), 500


@app.after_request
def _force_json_and_cors(response):
